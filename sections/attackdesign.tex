...
The \textbf{RGB-D Attack} module spoofs the liveness detection safeguarded by the RGB and depth simultaneously. First, we generate an RGB adversarial image using an evolutionary strategy to bypass the RGB liveness detection, and then we physically align projected scatter pattern with the forged RGB image to launch the uniform RGB-D attack.
