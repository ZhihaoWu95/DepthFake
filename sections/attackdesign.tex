...
\begin{algorithm}[t]
	...
	\label{alg}
	\begin{algorithmic}[1]
		\REQUIRE Printing-Capturing Pricess $C[P(\cdot)]$, the legitimate user's RGB image $X$, RGB-based liveness detection black-box model $M$ and its liveness  $Threshold$, perturbation uint size $(w, h)$
